...
**C.1 FINDING (2026-07, cggmp21 `=0.6.3`) — reconciled from the gate.** The pinned crate **does not implement identifiable abort or threshold (t-of-n) key refresh** (only non-threshold refresh). Two plan assumptions are corrected: (1) EVM-leg *cryptographic* attribution (S2, Phase F.1) is **not available from cggmp21 0.6.3** — for devnet/testnet the `Pevm` leg uses coarse detection (absence of a valid mint → session exclusion → S8 freeze; the freeze bounds loss without needing a named party), and the synedrion (CGGMP'24) vs. upstream-cggmp21 decision is deferred to *before mainnet*; (2) `Pevm` proactive refresh (Phase J.1) is a **fresh threshold DKG + aux-info + EVM `addSigner`/`removeSigner` re-point**, not an in-place refresh. The FROST (`Pgw`) leg keeps full per-share attribution (§18.5) and native refresh, so both controls survive on the gateway key. See `bridge/signer/DUE_DILIGENCE.md` → *Findings (blocking)*. Gate item (c) below is updated accordingly. **Progress:** (a) low-S normalisation **and** recovery-id assembly are implemented + unit-tested std-only (`bridge/signer/src/conformance.rs`); the `ecrecover` round-trip and (b) the libsodium pre-verify remain OPEN pending the vendored crates (they require a Rust toolchain + crates.io + libsodium dev headers on the build host).
...
Convert identifiable abort into economic security on the existing obligations machinery. The FROST (`Pgw`) leg is fully attributable (whitepaper §7, §18.5). The CGGMP21 (`Pevm`) leg is attributable **only with a library build that implements identifiable abort** — cggmp21 0.6.3 does not (C.1 finding), so it runs in coarse-detection mode until that is resolved; see F.1.

**F.1 Detection — EVM leg (CGGMP21).** *Target design (requires a cggmp21 build with identifiable abort):* a failed NIZK immediately attributes fault (S2); if all proofs verify but `δ` is malformed or the assembled signature is invalid, run the identification procedure: `Π_j Δ_j =? g^δ` and `Π_j S_j =? X^δ` localize the fault; parties open the implicated MtA ciphertexts; per-share `Γ^{σ_i} =? Δ̃_i^m S̃_i^r` pinpoints bad online shares. **⚠ C.1 finding — cggmp21 0.6.3 does not expose identifiable abort.** Until a build that does is adopted (synedrion/CGGMP'24 or upstream cggmp21 — decision due before mainnet), the `Pevm` leg runs in **coarse-detection mode**: a signing session that fails to produce a valid, `ecrecover`-checking mint is aborted, the non-cooperating members are excluded, the session retries with a fresh ≥`t+1` subset, and a persistent failure escalates to the S8 **freeze** (which bounds loss without a cryptographically named party). No `Pevm` slashing report is emitted in this mode. The gateway (`Pgw`/FROST) leg is unaffected and remains fully attributable (F.1′).
...
- **J.1 Epochal refresh of *both* keys (S7).** `Pgw` refreshes in place via a **FROST proactive Shamir refresh** (jointly add a verifiable sharing of zero, re-randomizing every share while preserving the group key) — public key unchanged → **no on-chain action**. **⚠ C.1 finding — cggmp21 0.6.3 has no threshold-key refresh**, so the earlier "`Pevm` CGGMP21 DH-based in-place refresh" is **not available**: refreshing `Pevm` requires a **fresh threshold DKG + aux-info generation over the committee, followed by an EVM `addSigner`/`removeSigner` re-point** to the new key (the same shape as a `(n,t)` change). This is heavier than the FROST leg and changes the on-chain footprint (the mint-authorizing key rotates, so the wBDX contract must be re-pointed), so `Pevm` refresh is scheduled on a **coarser cadence** than `Pgw` (e.g. per-`(n,t)`-epoch or on suspicion, not every 24h epoch) until a cggmp21 build with threshold refresh — or synedrion, which has native resharing — is adopted. Unused presignatures **and** nonce batches are erased on any refresh (S3). Stolen sub-`t+1` share sets of either key are invalidated. The two flows are independent.
...
| Corrupt EVM-leg signer (attribution) | §7 | *Target:* CGGMP21 identifiable abort → slash. **cggmp21 0.6.3 lacks it (C.1 finding)** → interim: coarse detection + session exclusion + S8 freeze (no named-party slash); FROST leg fully attributable | **S2/S8**; Phase F.1, C.1 |
