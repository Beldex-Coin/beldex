...
mod tests {
    ...
    /// C++ CROSS-CHECK (byte-for-byte match to `beldexd`). Fill these in from a tiny
    /// `beldexd` snippet — `crypto::generate_keys(...)` for two keypairs, then
    /// `encrypt_gateway_deposit_memo(plaintext, tx_sec, view_pub, output_index, ct)` —
    /// printing tx_public, view_secret, plaintext, output_index, ciphertext as hex.
    /// This test asserts our [`decrypt`] recovers the same plaintext from the C++
    /// ciphertext, pinning the port. Ignored until the vector is pasted in.
    #[test]
    #[ignore = "paste a beldexd-generated vector to pin the byte-for-byte match"]
    fn cpp_cross_check_vector() {
        ensure_init().unwrap();
        // let tx_public: [u8;32]  = hex!("…");
        // let view_secret: [u8;32] = hex!("…");
        // let output_index: u64    = /* … */;
        // let ciphertext: Vec<u8>  = hex!("…").to_vec();
        // let expected_plaintext   = hex!("…").to_vec();
