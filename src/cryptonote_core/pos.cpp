...
constexpr std::string_view round_state_string(round_state state)
{
  switch(state)
  {
    ...
    case round_state::wait_for_vrf_round: return "Wait For VRF Round"sv;
...
crypto::hash msg_signature_hash(crypto::hash const &top_block_hash, POS::message const &msg)
{
  ...
  switch(msg.type)
  {
    ...
    {
      auto buf = tools::memcpy_le(top_block_hash.data, msg.round, msg.vrf_proof.proof.data, msg.vrf_proof.key.data);
      result   = blake2b_hash(buf.data(), buf.size());
    }
    break;
    ...
    case POS::message_type::vrf_block_template:
    {
      ...
      auto buf = tools::memcpy_le(msg.round, block_hash.data, msg.vrf_block_template.proof.data, msg.vrf_block_template.key.data);
      result   = blake2b_hash(buf.data(), buf.size());
    }
    break;
...
void POS::handle_message(void *quorumnet_state, POS::message const &msg, bool all_mn)
{
      ...
      !msg_signature_check(msg, context.wait_for_next_block.top_hash, context.prepare_for_round.quorum, &sig_check_err))
  {
    ...
    if (print_err)
      MERROR("Signature check fail: " << sig_check_err);
  ...
  switch(msg.type)
  {
    ...
    case POS::message_type::vrf_block_template:
    {
      ...
      {
        MTRACE(log_prefix(context) << "Received unparsable POS VRF block template blob");
        return;
      }

      if (block.POS.round != context.prepare_for_round.round)
      {
        MTRACE(log_prefix(context) << "Received POS VRF block template specifying different round " << +block.POS.round
                                   << ", expected " << +context.prepare_for_round.round);
        return;
      }
...
round_state send_and_wait_for_vrf_proofs(round_context &context, void *quorumnet_state, master_nodes::master_node_keys const &key, cryptonote::Blockchain const &blockchain)
{
  ...
  if(process_vrf){
    ...
    if (context.transient.vrf_proof.send.one_time_only())
    {
      ...
      err = vrf_verify(output, reinterpret_cast<const unsigned char*>(key.pub.data), context.transient.vrf_proof.send.data.data, alpha_bytes, sizeof(crypto::hash));
      if (err != 0) {
...
round_state prepare_vrf_quorum(round_context &context, master_nodes::master_node_keys const &key, cryptonote::Blockchain const &blockchain)
{
  ...
  for (const auto &[pubkey, proof]  : quorum)
  {
    ...
    int err = vrf_verify(output, pk, proof.value().data, alpha_bytes, sizeof(crypto::hash));
    if (err != 0) {
  ...
  if (context.prepare_vrf_quorum.quorum.validators.size() > master_nodes::POS_VRF_QUORUM_NUM_VALIDATORS) {
    context.prepare_vrf_quorum.quorum.validators.resize(7);
  }
...
round_state wait_for_vrf_round(round_context &context, cryptonote::Blockchain const &blockchain)
{
  const auto curr_height = blockchain.get_current_blockchain_height(true /*lock*/);
  if (context.wait_for_next_block.height != curr_height)
  {
  ...
  {
    MGINFO_GREEN(log_prefix(context) << "We are a POS VRF validator, waiting fot the vrf block template from producer." << " position is " << context.prepare_vrf_quorum.my_quorum_position);
...
round_state send_vrf_block_template(round_context &context, void *quorumnet_state, master_nodes::master_node_keys const &key, cryptonote::Blockchain &blockchain)
{
  ...
  msg.vrf_block_template.proof  = context.transient.vrf_proof.send.data;
...
{
  handle_messages_received_early_for_vrf_proof(context.transient.wait_for_vrf_block_template.stage, quorumnet_state, blockchain);
...
void POS::main(void *quorumnet_state, cryptonote::core &core)
{
       ...
       last_state != context.state || last_state == round_state::null_state;)
  {
    ...
    switch (context.state)
    {
      ...
      case round_state::wait_for_vrf_round:
        context.state = wait_for_vrf_round(context, blockchain);
        break;
