...
block_vrf::block_vrf(const block_vrf& b) :
  block_header(b),
  miner_tx{b.miner_tx},
  tx_hashes{b.tx_hashes},
  signatures{b.signatures}
...
block_vrf::block_vrf(block_vrf&& b) :
  block_header(std::move(b)),
  miner_tx{std::move(b.miner_tx)},
  tx_hashes{std::move(b.tx_hashes)},
  signatures{std::move(b.signatures)}
...
block_vrf& block_vrf::operator=(const block_vrf& b)
{
  block_header::operator=(b);
  miner_tx = b.miner_tx;
  tx_hashes = b.tx_hashes;
...
block_vrf& block_vrf::operator=(block_vrf&& b)
{
  block_header::operator=(std::move(b));
  miner_tx = std::move(b.miner_tx);
  tx_hashes = std::move(b.tx_hashes);
...
block_vrf::block_vrf(const block& b) :
  block_header(b),
  miner_tx{b.miner_tx},
  tx_hashes{b.tx_hashes}
{
...
block_vrf& block_vrf::operator=(const block& b)
{
  block_header::operator=(b);
  miner_tx = b.miner_tx;
  tx_hashes = b.tx_hashes;
...
block_vrf& block_vrf::operator=(block&& b)
{
  block_header::operator=(std::move(b));
  miner_tx = std::move(b.miner_tx);
  tx_hashes = std::move(b.tx_hashes);
...
bool block_vrf::is_hash_valid() const
{
  return hash_valid.load(std::memory_order_acquire);
}
void block_vrf::set_hash_valid(bool v) const
{
  hash_valid.store(v,std::memory_order_release);
}
