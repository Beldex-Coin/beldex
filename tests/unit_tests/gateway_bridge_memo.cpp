...
namespace {
  ...
  TEST(GatewayBridgeMemo, cross_check_vector_for_rust)
  {
    auto to_hex = [](const unsigned char* p, size_t n) {
      ...
      std::string out;
      out.reserve(n * 2);
      for (size_t i = 0; i < n; ++i) { out += d[p[i] >> 4]; out += d[p[i] & 0xf]; }
      return out;
    };
    ...
    // chain_id = 1, a fixed 20-byte EVM address. The memo lands on output_index 0 of
    // a one-destination withdrawal, so the ciphertext below is exactly what consensus
    // would carry. (Going through construct_gateway_withdraw_tx rather than calling the
    // file-static encrypt_gateway_bridge_memo keeps this pinned to the shipped path.)
    crypto::eth_address evm{};
    for (int i = 0; i < 20; ++i) reinterpret_cast<unsigned char*>(&evm)[i] = static_cast<unsigned char>(0xa0 + i);

    gateway_withdraw_destination d{};
    d.gateway_id              = view_pub;   // the memo is encrypted to the DESTINATION gateway id
    d.amount                  = 1000;
    d.gateway_bridge_chain_id = 1;
    d.gateway_bridge_evm_addr = evm;

    transaction tx{};
    crypto::hash hash_to_sign{};
    ASSERT_TRUE(construct_gateway_withdraw_tx(HF, NET, random_gateway_keys().pub, {d}, /*fee=*/10, tx, hash_to_sign));

    // construct_gateway_withdraw_tx picks its own tx key, so read back the pair that
    // actually produced the ciphertext rather than the seeds above.
    const crypto::public_key actual_tx_pub = get_tx_pub_key_from_extra(tx);
    const auto m = only_memo(tx);

    // The expected plaintext: chain_id (8B LE) ‖ evm_addr (20B) ‖ 4 zero bytes.
    unsigned char expect_plain[32] = {0};
    expect_plain[0] = 1; // chain_id = 1, little-endian
    std::memcpy(expect_plain + 8, &evm, sizeof(evm));

    std::cout << "\n=== gateway bridge memo cross-check vector (paste into gateway_memo::cpp_cross_check_vector) ===\n"
              << "tx_public    = " << to_hex(reinterpret_cast<const unsigned char*>(&actual_tx_pub), 32) << "\n"
              << "view_secret  = " << to_hex(reinterpret_cast<const unsigned char*>(view_sec.data), 32) << "\n"
              << "output_index = " << m.output_index << "\n"
              << "plaintext    = " << to_hex(expect_plain, sizeof(expect_plain)) << "\n"
              << "ciphertext   = " << to_hex(reinterpret_cast<const unsigned char*>(m.ciphertext.data), 32) << "\n"
              << "NOTE: tx_public is freshly random per run; the Rust vector must be pasted from ONE run.\n"
              << "==============================================================================================\n";

    // Self-check the C++ round-trip (independent of the printed vector).
    gateway_bridge_memo_plaintext out{};
    ASSERT_TRUE(decrypt_gateway_bridge_memo(tx, m, view_sec, out));
    EXPECT_EQ(out.chain_id, 1u);
    EXPECT_EQ(0, std::memcmp(&out.evm_addr, &evm, sizeof(evm)));
  }
